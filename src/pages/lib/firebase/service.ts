...
export async function signIn(userData: {email: string}) {
    ...
    // di bawah ini const data untuk mengecek apakah data nya ada apa agak
    const data = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
    }));
...
export async function signInWithGoogle(userData: any, callback: any) {
    ...
    if(data.length > 0) {
        ...
        await updateDoc(doc(firestore, "users", data[0].id), userData).then(() => {
            ...
            callback({status: false, message: "Sign in with google failed", data: userData})
        })
    } else {
        userData.role = "member";
        await addDoc(collection(firestore, "users"), userData)
        .then(() => {
            callback({status: true, message: "Sign in with google succes"})
        }).catch(() => {
